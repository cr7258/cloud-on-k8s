...
func NewStartupProbe() *corev1.Probe {
	return &corev1.Probe{
		FailureThreshold:    60,
		InitialDelaySeconds: 10,
		PeriodSeconds:       10,
		...
		ProbeHandler: corev1.ProbeHandler{
			Exec: &corev1.ExecAction{
				Command: []string{"bash", "-c", path.Join(volume.ScriptsVolumeMountPath, StartupProbeScriptConfigKey)},
			},
		},
	}
}

const StartupProbeScriptConfigKey = "startup-probe-script.sh"
const StartupProbeScript = `#!/usr/bin/env bash

# fail should be called as a last resort to help the user to understand why the probe failed
function fail {
  timestamp=$(date --iso-8601=seconds)
  echo "{\"timestamp\": \"${timestamp}\", \"message\": \"startup probe failed\", "$1"}" | tee /proc/1/fd/2 2> /dev/null
  exit 1
}
...
STARTUP_PROBE_TIMEOUT=${STARTUP_PROBE_TIMEOUT:=3}
...
# request Elasticsearch on /
# we are turning globbing off to allow for unescaped [] in case of IPv6
ENDPOINT="${STARTUP_PROBE_PROTOCOL:-https}://${LOOPBACK}:9200/_cluster/health"
ORIGIN_HEADER="` + http.InternalProductRequestHeaderString + `"
cluster_health=$(curl --max-time ${STARTUP_PROBE_TIMEOUT} -H "${ORIGIN_HEADER}" -XGET -g -s -k ${BASIC_AUTH} $ENDPOINT)
curl_rc=$?
...
# ready if status is green
status=$(echo ${cluster_health} | grep -oP '"status":"\K[^"]+')
